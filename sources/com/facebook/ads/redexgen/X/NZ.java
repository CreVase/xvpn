package com.facebook.ads.redexgen.X;

import android.content.res.Configuration;
import android.widget.FrameLayout;
import android.widget.LinearLayout;
import android.widget.RelativeLayout;

/* loaded from: assets/audience_network.dex */
public final class NZ extends RelativeLayout {
    public static String[] A06 = {"GfXFZtEZk1NZfob7Kzf1xTq0EESlD7si", "ewzI1iE8tF5Z2B86REyPWWZDw7DKLBH9", "kOyZSgqEibXIKTIW7tLvn8rBuxcmsyDd", "XpDxTY", "v6DhXkGtrQWw75h", "XA27k4WXBENWs2k6KViFhCSGxV0pMN8g", "cLUj8uWE8C4NOcjwK2waVMiWyPuWQVsm", ""};
    public final int A00;
    public final LinearLayout A01;
    public final C0922aQ A02;
    public final YA A03;
    public final InterfaceC0444If A04;
    public final InterfaceC0531Lt A05;

    public NZ(YA ya, C0922aQ c0922aQ, InterfaceC0444If interfaceC0444If, InterfaceC0531Lt interfaceC0531Lt, int i, int i2) {
        super(ya);
        this.A03 = ya;
        this.A02 = c0922aQ;
        this.A04 = interfaceC0444If;
        this.A05 = interfaceC0531Lt;
        this.A00 = i;
        LinearLayout linearLayout = new LinearLayout(ya);
        this.A01 = linearLayout;
        A00();
        addView(linearLayout, new FrameLayout.LayoutParams(-1, -1));
        setLayoutOrientation(i2);
    }

    private void A00() {
        int i = 0;
        while (true) {
            C0922aQ c0922aQ = this.A02;
            if (A06[4].length() != 15) {
                throw new RuntimeException();
            }
            String[] strArr = A06;
            strArr[6] = "6kBjYVyQvbEVjVKJKeawSG2VEKMXDcJL";
            strArr[5] = "aQimcOJmMOk7TFcbK4Cqi1kaRliNN4Qj";
            int i2 = c0922aQ.A0q();
            if (i < i2) {
                T7 t7 = new T7(this.A03, this.A02.A0t(i), this.A04, this.A05);
                LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(-1, -1);
                layoutParams.weight = 1.0f;
                int i3 = T7.A0B;
                int i4 = T7.A0B;
                int i5 = T7.A0B;
                int i6 = T7.A0B;
                layoutParams.setMargins(i3, i4, i5, i6);
                t7.setLayoutParams(layoutParams);
                this.A01.addView(t7);
                i++;
            } else {
                return;
            }
        }
    }

    @Override // android.view.View
    public final void onConfigurationChanged(Configuration configuration) {
        super.onConfigurationChanged(configuration);
        setLayoutOrientation(configuration.orientation);
    }

    private void setLayoutOrientation(int i) {
        if (i != 1) {
            this.A01.setOrientation(0);
            LinearLayout linearLayout = this.A01;
            int i2 = this.A00;
            linearLayout.setPadding(0, i2, 0, (int) (i2 * 0.25d));
            return;
        }
        this.A01.setOrientation(1);
        LinearLayout linearLayout2 = this.A01;
        int i3 = this.A00;
        linearLayout2.setPadding(0, (int) (i3 * 1.5d), 0, i3);
    }
}
