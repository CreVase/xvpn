...
public final class Huffman {
    ...
    public final void decode(sr srVar, long j, rr rrVar) {
        Node node = root;
        int i = 0;
        long j2 = 0;
        int i2 = 0;
        while (j2 < j) {
            j2++;
            i = (i << 8) | Util.and(srVar.readByte(), 255);
            i2 += 8;
            while (i2 >= 8) {
                node = node.getChildren()[(i >>> (i2 - 8)) & 255];
                if (node.getChildren() == null) {
                    rrVar.k(node.getSymbol());
        ...
        while (i2 > 0) {
            Node node2 = node.getChildren()[(i << (8 - i2)) & 255];
            if (node2.getChildren() == null && node2.getTerminalBitCount() <= i2) {
                rrVar.k(node2.getSymbol());
    ...
    public final void encode(at atVar, rr rrVar) throws IOException {
        int c = atVar.c();
        long j = 0;
        int i = 0;
        int i2 = 0;
        while (i < c) {
            int i3 = i + 1;
            int and = Util.and(atVar.f(i), 255);
            int i4 = CODES[and];
        ...
        if (i2 > 0) {
            rrVar.k((int) ((j << (8 - i2)) | (255 >>> i2)));
        }
    }

    public final int encodedLength(at atVar) {
        long j = 0;
        for (int i = 0; i < atVar.c(); i++) {
            j += CODE_BIT_COUNTS[Util.and(atVar.f(i), 255)];
        }
        return (int) ((j + 7) >> 3);
    }
