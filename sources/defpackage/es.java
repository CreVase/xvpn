...
public final class es {
    ...
    private static long getChannelFieldOffset(Class<?> cls) {
        if (cls != null) {
            try {
                if (n1.hasUnsafeArrayOperations()) {
                    return n1.objectFieldOffset(cls.getDeclaredField("channel"));
                }
                return -1L;
    ...
    private static boolean writeToChannel(ByteBuffer byteBuffer, OutputStream outputStream) throws IOException {
        ...
        if (j >= CHANNEL_FIELD_OFFSET && FILE_OUTPUT_STREAM_CLASS.isInstance(outputStream)) {
            try {
                writableByteChannel = (WritableByteChannel) n1.getObject(outputStream, j);
